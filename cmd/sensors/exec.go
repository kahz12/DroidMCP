...
const (
	...
	locationExecTimeout = 30 * time.Second
	maxExecTimeout      = 120 * time.Second
	maxOutputBytes      = 1 << 20 // 1 MiB per stream

	binBattery  = "termux-battery-status"
	binLocation = "termux-location"
	binWifiInfo = "termux-wifi-connectioninfo"
	binVolume   = "termux-volume"
	binSensor   = "termux-sensor"
...
// sensorResult is the captured outcome of running a sensor backend command.
...
// runSensorCmd invokes a backend command, capturing stdout and stderr
// separately into capped buffers and applying the per-call timeout.
func runSensorCmd(ctx context.Context, name string, args []string, timeout time.Duration) (*sensorResult, error) {
	if timeout <= 0 {
