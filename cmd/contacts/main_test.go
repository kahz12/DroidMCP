...
func captureArgs(t *testing.T, dir string) func() string {
	...
	t.Setenv("CONTACTS_ARGS_FILE", argsFile)
	return func() string {
...
// recordArgv writes the received argv then emits an empty contact list, so the
// backend command still "succeeds" while the test inspects what it was called
// with.
const recordArgv = `printf '%s\n' "$@" > "$CONTACTS_ARGS_FILE"; echo '[]'`
...
func TestEnsureBinariesMissingMessage(t *testing.T) {
	...
	err := ensureBinaries(binContactList)
	if err == nil {
		t.Fatal("expected error when binary is missing")
	}
	if !strings.Contains(err.Error(), "termux-api") || !strings.Contains(err.Error(), binContactList) {
